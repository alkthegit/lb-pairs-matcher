...
class Item {
  /**
   *
   * @param {number} id Идентификатор объекта в массиве подобных объектов
   * @param {ItemStates} state Состояние объекта: активен, неактивен, угадан
   * @param {number} pairId Идентификатор пары для объекта в общем массиве подобных объектов
   */
...
export default class PairsMatcher {
  constructor() {
    ...
    /**
     * Последовательность ходов, ведущая к победе - упорядоченный массив идентификаторов
     * объектов в порядке угадывания
     *
     * @type {number[]}
     */
    this.winStrategy = [];
   ...
   * @param {ItemInfo[]} itemsMap Исходная раскладка объектов в наборе объектов -
   * для последующего размещения на игровом поле
   * @return {Promise<number[]>} - Массив перемешанных идентификаторов из массива itemsMap
   * для раскладки в новой игре
   */
  newGame(itemsMap) {
    return new Promise((resolve, reject) => {
      try {
        if (!itemsMap || itemsMap.length === 0) {
          throw new Error(
            `Необходимо передать исходную раскладку объектов, получено значение: '${itemsMap}'`,
          );
        }

        if (itemsMap.length % 2 !== 0) {
          throw new Error(
            `Число объектов в наборе должно быть четный, передано значение: '${itemsMap}'`,
          );
        }
        ...
        itemsMap.forEach((item) => {
          this.items.push(new Item(item.id, ItemStates.inactive, item.pairId));
        });
        ...
        this.winStrategy = [...this.items]
          .map(item => ({ id: item.id, pairId: item.pairId }))
          .sort((prev, next) => {
            ...
            if (prev.pairId === next.pairId) {
              ...
              // в теории невозможный случай, когда идентификатор объекта не уникальный
              // в наборе объектов
              return 0;
            }
            // в теории невозможный случай, когда идентификатор пары объектов не уникальный
            // в наборе пар
            return 0;
          });
        ...
        // на выходе получился массив с перемешанными позициями исходного массива.
        // Клиент данного метода
        // может обращаться к своему исходному массиву при помощи данного набора:
        // Исходный_массив[itemIndexes[index]]
        resolve(itemIds);
      } catch (err) {
        reject(err);
      }
   ...
   * Так же проверяет, не был ли уже активирован объект-пара.
   * Если был, то помечает объекты угаланными и испускает событие PairsMatcherEvents.match
  ...
  selectItem(id) {
    ...
    if (item.state !== ItemStates.inactive) {
      return;
    }
    ...
    if (this.selectedItemId !== -1) {
      ...
      if (selectedItem.pairId === item.pairId) {
        ...
        item.state = ItemStates.inactive;
        selectedItem.state = ItemStates.inactive;
        this.emit(PairsMatcherEvents.Active, item.id);
      ...
      item.state = ItemStates.active;
      this.emit(PairsMatcherEvents.Active, item.id);
      this.gameStats.tries = this.gameStats.tries + 1;
    }
